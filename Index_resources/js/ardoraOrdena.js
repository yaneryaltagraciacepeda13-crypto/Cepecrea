...
} else {$("#ardoraQuest").html(vHtml+"<img alt='Play' id='playSound' class='imaLeft' src='Index_resources/media/' alt='Sound' align='left'>"+ "<p>" + questionW[indexG] + "</p>");
...
$("#cont_ima").html("");if (imageW[indexG].localeCompare("") != 0) {$("#cont_ima").html("<img id='imaIz' alt='" + alterW[indexG] + "' src='" + dirMedia + imageW[indexG] + "'/>");}
$("#imagenFondo").html("");if (imageW[indexG].localeCompare("") != 0) {$("#imagenFondo").html("<img id='imagenModal' alt='" + alterW[indexG] + "' src='" + dirMedia + imageW[indexG] + "'/>");if (informaW[indexG].localeCompare("") != 0) {
$("#imagenFondo").append('<button id="ifbut" tabindex="0">🛈</button>');$("#imagenFondo").append('<div id="infoText" class="infoText">'+informaW[indexG]+'</div>');
...
document.getElementById("imaIz").addEventListener("click", function() {document.getElementById("modal").style.display = "flex";});
document.getElementById("modal").addEventListener("click", function(e) {if (e.target !== document.getElementById("imagenModal")) {document.getElementById("modal").style.display = "none";}});
document.getElementById("cont_ima").addEventListener("keydown", function(e) {if (e.key !== "Tab") {document.getElementById("modal").focus();document.getElementById("modal").style.display = "flex";}});
document.getElementById("modal").addEventListener("keydown", function(e) {if (e.key === "Escape") {e.preventDefault();document.getElementById("modal").style.display = "none";}});
}}
...
function randomSort() {var iW=[];var iWc=[];for (i = 0; i < words.length; i++) {if (words[i].localeCompare("") != 0) {iW.push(words[i]);iWc.push(c[i]);wordsG.push("");cG.push(0);imageW.push("");questionW.push("");audioW.push("");alterW.push("");informaW.push("");}}
...
wordsG[ind] = iW[i];cG[ind]=iWc[i];imageW[ind] = imaW[i];questionW[ind] = queW[i];audioW[ind] = auW[i];alterW[ind]=altW[i];informaW[ind]=infoW[i];}}
...
$("#ardoraActCanvas").attr("tabindex","0");$("#ardoraActCanvas").attr("aria-label",textBNext);$("#ardoraActCanvas").focus();$("#ardoraActCanvas").keydown(function(e){$("#ardoraActCanvas").click();});
